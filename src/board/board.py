...
class Board:
    ...
    def placeShips(self, player_type: int):
        ...
        self.__placeShips(
            self.__destroyer.ship_registry, self.__destroyer.size, player_type
        )
        self.__placeShips(
            self.__cruiser.ship_registry, self.__cruiser.size, player_type
        )
        self.__placeShips(
            self.__submarine.ship_registry, self.__submarine.size, player_type
        )
        self.__placeShips(
            self.__battleship.ship_registry, self.__battleship.size, player_type
        )
        self.__placeShips(
            self.__carrier.ship_registry, self.__carrier.size, player_type
        )
    ...
    def __placeShips(self, ship_registry: str, ship_size: int, player_type: int):
        ...
        while True:
            ...
            try:
                row = (
                    int(input("Enter the ship's starting row: "))
                    if (player_type == PlayerType.player.value)
                    else random.randint(0, 9)
                )
                column = (
                    int(input("Enter the ship's starting column: "))
                    if (player_type == PlayerType.player.value)
                    else random.randint(0, 9)
                )
                orientation = (
                    int(
                        input(
                            "Enter the ship's orientation (1. Horizontal | 2. Vertical): "
                        )
                    )
                    if (player_type == PlayerType.player.value)
                    else random.randint(1, 2)
                )
                if self.__validatePositioningParameters(
                    ship_registry, row, column, orientation, ship_size, False
                ):
                    self.__validatePositioningParameters(
                        ship_registry, row, column, orientation, ship_size, True
                    )
                    if player_type == PlayerType.player.value:
                        self.ships_board
                    break
                    ...
                    print(
                        "The values ​​entered are invalid for one of the following reasons: "
                    )
                    print(
                        "-> A ship is in a position that obstructs the selected values."
                    )
                    print("-> The data entered exceeds the size of the dashboard.")
                    print(
                        "Double-check your dashboard and select other values.",
                        end="\n\n",
                    )
    ...
    def __validatePositioningParameters(
        self,
        ship_registry: str,
        row: int,
        column: int,
        ...
        while ship_size > 0:
            if self.__ships_board[row][column] == "v":
                self.__ships_board[row][column] = (
                    f"{ship_registry}"
                    if (validate_change)
                    else self.__ships_board[row][column]
                )
                column = (
                    (column + 1)
                ...
                row = (
                    (row + 1) if (orientation == Orientation.vertical.value) else (row)
                )
                ship_size -= 1
            else:
                print(
                    "The selected row, column, and orientation are not valid.",
                    end="\n\n",
                )
                break
    ...
    def positionAttack(self, player_type: int) -> list:
        ...
        while True:
            try:
                row = (
                    int(input("Enter the row where you want to launch the attack: "))
                    if (player_type == PlayerType.player.value)
                    else random.randint(0, 9)
                )
                column = (
                    int(input("Enter the column where you want to launch the attack: "))
                    if (player_type == PlayerType.player.value)
                    else random.randint(0, 9)
                )
                if (row > -1 and row < 10) and (column > -1 and column < 10):
                    ...
                    print(
                        "The position you selected is not a valid position for a 10 x 10 board."
                    )
            except ValueError as e:
    ...
    def hasLiveShips(self) -> bool:
        return (
            True
            if (
                self.__destroyer.isAlive
                and self.__cruiser.isAlive
                and self.__submarine.isAlive
                and self.__battleship.isAlive
                and self.__carrier.isAlive
            )
            else False
        )
    ...
    def __receiveEnemyAttack(self, attack: list) -> list:
        ...
        self.__ships_board[row][column] = (
            "x" if (self.__ships_board[row][column] == "v") else "o"
        )
        attack_result_text = (
            "Attack failed"
            if (self.__ships_board[row][column] == "v")
